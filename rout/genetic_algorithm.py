...
def simulate_car(
    ...
    env_x_bounds: Tuple[float, float] = (-10.0, 110.0),
    ...
    mass = (
        mass_base
        + motor_mass_coeff * genome.motor_power
        + fuel_mass_coeff * genome.fuel_tank
    )
    ...
    for step in range(max_steps):
        ...
        if (
            not hasattr(genome, "nn_weights")
            or len(genome.nn_weights) != NN_WEIGHTS_SIZE
        ):
            ...
            w = genome.nn_weights
            p = 0
            W1 = w[p : p + NN_HIDDEN * NN_INPUT].reshape((NN_HIDDEN, NN_INPUT))
            p += NN_HIDDEN * NN_INPUT
            b1 = w[p : p + NN_HIDDEN]
            p += NN_HIDDEN
            W2 = w[p : p + NN_OUTPUT * NN_HIDDEN].reshape((NN_OUTPUT, NN_HIDDEN))
            p += NN_OUTPUT * NN_HIDDEN
            b2 = w[p : p + NN_OUTPUT]
            p += NN_OUTPUT
...
def fitness_of(genome: CarGenome, goal_x: float = 100.0, obstacles=None) -> float:
    ...
    """
    x_reached, collision, traj = simulate_car(
        genome, goal_x=goal_x, obstacles=obstacles
    )
...
def sample_potential(field: dict, x: float, y: float) -> float:
    ...
    if (
        not np.isfinite(v00)
        and not np.isfinite(v10)
        and not np.isfinite(v01)
        and not np.isfinite(v11)
    ):
        return float("inf")
    BIG = 1e6
    vals = [
        v00 if np.isfinite(v00) else BIG,
...
class GeneticAlgorithm:
    def __init__(
        ...
        try:
            self.potential_field = compute_potential_field(
                self.obstacles,
    ...
    def _evaluate_population(self) -> np.ndarray:
        ...
        for i in range(self.population_size):
            ...
            x_reached, collision, traj = simulate_car(
                genome, goal_x=self.goal_x, obstacles=self.obstacles
            )
            steps = max(0, len(traj) - 1)
            time_cost = 0.02
    ...
    def run(
        self, generations: int = 100, verbose: bool = True, on_generation=None
    ) -> Tuple[Optional[CarGenome], float]:
        ...
        for gen in range(int(generations)):
            ...
            if fitness[idx_best] > best_fitness:
                ...
                best_genome = CarGenome.from_vector(
                    self.pop[idx_best].copy(), NN_WEIGHTS_SIZE
                )

            if verbose and (
                gen % max(1, int(generations) // 10) == 0 or gen == int(generations) - 1
            ):
